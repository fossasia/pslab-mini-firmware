...
#define VECT_TAB_OFFSET                                                        \
    0xC000U /*!< Vector Table base offset field.                               \
...
uint8_t const AHBPrescTable[16] =
...
/* FPU settings ------------------------------------------------------------*/
    ...
    /* Reset HSEON, HSECSSON, HSEBYP, HSEEXT, HSIDIV, HSIKERON, CSION, CSIKERON,
     * HSI48 and PLLxON bits */
#if defined(RCC_CR_PLL3ON)
    RCC->CR &=
        ~(RCC_CR_HSEON | RCC_CR_HSECSSON | RCC_CR_HSEBYP | RCC_CR_HSEEXT |
          RCC_CR_HSIDIV | RCC_CR_HSIKERON | RCC_CR_CSION | RCC_CR_CSIKERON |
          RCC_CR_HSI48ON | RCC_CR_PLL1ON | RCC_CR_PLL2ON | RCC_CR_PLL3ON);
#else
    RCC->CR &=
        ~(RCC_CR_HSEON | RCC_CR_HSECSSON | RCC_CR_HSEBYP | RCC_CR_HSEEXT |
          RCC_CR_HSIDIV | RCC_CR_HSIKERON | RCC_CR_CSION | RCC_CR_CSIKERON |
          RCC_CR_HSI48ON | RCC_CR_PLL1ON | RCC_CR_PLL2ON);
#endif
 ...
 * HSE_VALUE(***) or HSI_VALUE(**) or CSI_VALUE(*) multiplied/divided by the PLL
 * factors.
 *
 *         (*) CSI_VALUE is a constant defined in stm32h5xx_hal.h file (default
 * value 4 MHz) but the real value may vary depending on the variations in
 * voltage and temperature.
 *
 *         (**) HSI_VALUE is a constant defined in stm32h5xx_hal.h file (default
 * value 64 MHz) but the real value may vary depending on the variations in
 * voltage and temperature.
 *
 *         (***) HSE_VALUE is a constant defined in stm32h5xx_hal.h file
