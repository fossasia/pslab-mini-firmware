 ...
 * defines the API for initializing the UART peripheral, managing transmission
 * and reception, and registering callback functions for UART events (TX
 * complete, RX complete, idle line detection).
 */
...
/**
 * @brief Initialize the UART peripheral and start interrupt-based reception.
 *
...
/**
 * @brief Get the current write position in the RX buffer.
 *
...
/**
 * @brief Check if a UART TX transfer is currently in progress.
 *
 * @param bus UART bus instance
 ...
 * @param bus UART bus instance
 * @param dma_pos Current position in reception buffer
 */
