 ...
 *
 * This implementation relies on hardware-specific functions defined in the
 * platform UART low-level driver.
 *
...
{
    if (!handle || !handle->initialized || UART_LL_tx_busy(handle->bus_id) ||
