...
{
    return (int)_read_r(_impure_ptr, fd, buf, (size_t)count);
}

int _write(int fd, char const *buf, int count)
{
    return (int)_write_r(_impure_ptr, fd, buf, (size_t)count);
}

int _fstat(int fd, struct stat *st) { return _fstat_r(_impure_ptr, fd, st); }

int _isatty(int fd) { return _isatty_r(_impure_ptr, fd); }
