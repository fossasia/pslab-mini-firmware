 ...
 * 
 * This file provides stub implementations for system calls that newlib requires.
 * These stubs are minimal implementations that return error codes, allowing the
 * program to compile and link successfully.
 * 
...
_ssize_t _read_r(struct _reent *r, int fd, void *buf, size_t cnt) {
    (void)r; (void)fd; (void)buf; (void)cnt;
    errno = ENOSYS;
    return -1;
}
